<template>
    <div class="relative inline-block text-left">
        <div>
            <button @click="isExpanded" type="button" class="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm" id="menu-button" aria-expanded="true" aria-haspopup="true">
            <svg class="h-5 w-5" viewBox="0 0 21 21" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            
                <title>UPLB Apps</title>
                <desc>Created with Sketch.</desc>
                <defs>

                </defs>
                <g id="Page-1" stroke="none" stroke-width="1" fill="currentColor" fill-rule="evenodd">
                    <g id="Dribbble-Light-Preview" transform="translate(-219.000000, -200.000000)" fill="#000000">
                        <g id="icons" transform="translate(56.000000, 160.000000)">
                            <path d="M181.9,54 L179.8,54 C178.63975,54 177.7,54.895 177.7,56 L177.7,58 C177.7,59.105 178.63975,60 179.8,60 L181.9,60 C183.06025,60 184,59.105 184,58 L184,56 C184,54.895 183.06025,54 181.9,54 M174.55,54 L172.45,54 C171.28975,54 170.35,54.895 170.35,56 L170.35,58 C170.35,59.105 171.28975,60 172.45,60 L174.55,60 C175.71025,60 176.65,59.105 176.65,58 L176.65,56 C176.65,54.895 175.71025,54 174.55,54 M167.2,54 L165.1,54 C163.93975,54 163,54.895 163,56 L163,58 C163,59.105 163.93975,60 165.1,60 L167.2,60 C168.36025,60 169.3,59.105 169.3,58 L169.3,56 C169.3,54.895 168.36025,54 167.2,54 M181.9,47 L179.8,47 C178.63975,47 177.7,47.895 177.7,49 L177.7,51 C177.7,52.105 178.63975,53 179.8,53 L181.9,53 C183.06025,53 184,52.105 184,51 L184,49 C184,47.895 183.06025,47 181.9,47 M174.55,47 L172.45,47 C171.28975,47 170.35,47.895 170.35,49 L170.35,51 C170.35,52.105 171.28975,53 172.45,53 L174.55,53 C175.71025,53 176.65,52.105 176.65,51 L176.65,49 C176.65,47.895 175.71025,47 174.55,47 M167.2,47 L165.1,47 C163.93975,47 163,47.895 163,49 L163,51 C163,52.105 163.93975,53 165.1,53 L167.2,53 C168.36025,53 169.3,52.105 169.3,51 L169.3,49 C169.3,47.895 168.36025,47 167.2,47 M181.9,40 L179.8,40 C178.63975,40 177.7,40.895 177.7,42 L177.7,44 C177.7,45.105 178.63975,46 179.8,46 L181.9,46 C183.06025,46 184,45.105 184,44 L184,42 C184,40.895 183.06025,40 181.9,40 M174.55,40 L172.45,40 C171.28975,40 170.35,40.895 170.35,42 L170.35,44 C170.35,45.105 171.28975,46 172.45,46 L174.55,46 C175.71025,46 176.65,45.105 176.65,44 L176.65,42 C176.65,40.895 175.71025,40 174.55,40 M169.3,42 L169.3,44 C169.3,45.105 168.36025,46 167.2,46 L165.1,46 C163.93975,46 163,45.105 163,44 L163,42 C163,40.895 163.93975,40 165.1,40 L167.2,40 C168.36025,40 169.3,40.895 169.3,42" id="grid-[#1526]">

                            </path>
                        </g>
                    </g>
                </g>
            </svg>
            </button>
        </div>
        <Transition    
            enter-active-class="duration-300 ease-out"
            enter-from-class="transform opacity-0"
            enter-to-class="opacity-100"
            leave-active-class="duration-200 ease-in"
            leave-from-class="opacity-100"
            leave-to-class="transform opacity-0">
            <div v-show="expanded" class="absolute right-0 z-10 mt-4 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                <div class="flex justify-center py-1 grid grid-cols-3" role="none">
                    <a v-show="isAmisAdmin || isAmisFaculty || isAmisStudent || isAmisReps || isAmisSuperAdmin" href="https://amis.uplb.edu.ph" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">
                        <span><img src="@/assets/images/uplb-logo.png" alt=""></span>
                        <span class="text-center"> <p>AMIS</p> </span>
                    </a>
                    <a v-show="isOrsCollegeHead || isOrsFaculty || isOrsOvcre || isOrsReps || isOrsUnitAdmin || isOrsUnitHead" href="http://localhost:3002" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1">
                        <span><img src="@/assets/images/uplb-logo.png" alt=""></span>
                        <span class="text-center"> <p>ORS</p> </span>
                    </a>
                    <a v-show="isIamsAdmin" href="http://localhost:3000" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-2">
                        <span><img src="@/assets/images/uplb-logo.png" alt=""></span>
                        <span class="text-center"> <p>IAMS</p> </span>
                    </a>
                    <a v-show="isIamsAdmin" href="http://localhost:3001" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-2">
                        <span><img src="@/assets/images/uplb-logo.png" alt=""></span>
                        <span class="text-center"><p>IAMS - Visitors</p></span>
                    </a>
                    <a href="#" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-2">
                        <span><img src="@/assets/images/uplb-logo.png" alt=""></span>
                        <span class="text-center"><p>HRDO</p></span>
                    </a>
                    <a href="#" class="text-gray-700 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-2">
                        <span><img src="@/assets/images/uplb-logo.png" alt=""></span>
                        <span class="text-center"><p>OHMS</p></span>
                    </a>
                </div>
            </div>
        </Transition>
    </div>
</template>
<script>
export default {
    data(){
        return {
            expanded: false,
            isCurrent: true
        }
    },
    methods: {
        isExpanded() {
            this.expanded = !this.expanded
        }
    },
    computed: {
        //AMIS
        async isAmisAdmin() {
            if(this.$auth.user.resource_access['uplb-amis']){
                const roles = await this.$auth.user.resource_access['uplb-amis'].roles;
                return roles.find(el => el === "admin") ? true : false;
            } else {
                return false
            }
            
            
        },
        async isAmisStudent() {
            if(this.$auth.user.resource_access['uplb-amis']){
                const roles = await this.$auth.user.resource_access['uplb-amis'].roles;
                return roles.find(el => el === "student") ? true : false;
            } else {
                return false
            }
        },
        async isAmisFaculty() {
            if(this.$auth.user.resource_access['uplb-amis']){
                const roles = await this.$auth.user.resource_access['uplb-amis'].roles;
                return roles.find(el => el === "faculty") ? true : false;
            } else {
                return false
            }
        },
        async isAmisReps() {
            if(this.$auth.user.resource_access['uplb-amis']){
                const roles = await this.$auth.user.resource_access['uplb-amis'].roles;
                return roles.find(el => el === "reps") ? true : false;
            } else {
                return false
            }
        },
        async isAmisSuperAdmin() {
            if(this.$auth.user.resource_access['uplb-amis']){
                const roles = await this.$auth.user.resource_access['uplb-amis'].roles;
                return roles.find(el => el === "super_admin") ? true : false;
            } else {
                return false
            }
        },
        //ORS
        async isOrsUnitAdmin() {
            if(this.$auth.user.resource_access['uplb-ors']){
                const roles = await this.$auth.user.resource_access['uplb-ors'].roles;
                return roles.find(el => el === "unit_admin") ? true : false;
            } else {
                return false
            }
        },
        async isOrsOvcre() {
            if(this.$auth.user.resource_access['uplb-ors']){
                const roles = await this.$auth.user.resource_access['uplb-ors'].roles;
                return roles.find(el => el === "ovcre_admin") ? true : false;
            } else {
                return false
            }
        },
        async isOrsCollegeHead() {
            if(this.$auth.user.resource_access['uplb-ors']){
                const roles = await this.$auth.user.resource_access['uplb-ors'].roles;
                return roles.find(el => el === "college_head") ? true : false;
            } else {
                return false
            }
        },
        async isOrsFaculty() {
            if(this.$auth.user.resource_access['uplb-ors']){
                const roles = await this.$auth.user.resource_access['uplb-ors'].roles;
                return roles.find(el => el === "faculty") ? true : false;
            } else {
                return false
            }
        },
        async isOrsReps() {
            if(this.$auth.user.resource_access['uplb-ors']){
                const roles = await this.$auth.user.resource_access['uplb-ors'].roles;
                return roles.find(el => el === "reps") ? true : false;
            } else {
                return false
            }
        },
        async isOrsUnitHead() {
            if(this.$auth.user.resource_access['uplb-ors']){
                const roles = await this.$auth.user.resource_access['uplb-ors'].roles;
                return roles.find(el => el === "unit_head") ? true : false;
            } else {
                return false
            }
        },
        //IAMS
        async isIamsAdmin() {
            if(this.$auth.user.resource_access['uplb-iams']){
                const roles = await this.$auth.user.resource_access['uplb-iams'].roles;
                return roles.find(el => el === "admin") ? true : false;
            } else {
                return false
            }
        },
        //IAMS VISITORS
        async isIamsVisitorAdmin(){
            if(this.$auth.user.resource_access['uplb-iams-visitor']){
                const roles = await this.$auth.user.resource_access['uplb-iams-visitor'].roles;
                return roles.find(el => el === "admin") ? true : false;
            } else {
                return false
            }
        },
        async isIamsVisitor(){
            if(this.$auth.user.resource_access['uplb-iams-visitor']){
                const roles = await this.$auth.user.resource_access['uplb-iams-visitor'].roles;
                return roles.find(el => el === "visitor") ? true : false;
            } else {
                return false
            }
        },
    }
}
</script>